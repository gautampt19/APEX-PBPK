...
└────────────────────────────────────────┘"""
    ...
    # ── Step 4: Validate → Auto-Repair → Re-validate → Fallback ──────────
    r_script_to_run = r_script_llm
    use_fallback    = False

    if not os.path.exists(r_script_llm):
        print("⚠️  LLM did not produce an R script. Falling back to template generator …")
        use_fallback = True
    else:
        ...
        if valid:
            ...
            if repaired:
                valid2, err2 = validate_r_syntax(r_script_llm)
                if valid2:
                ...
                print("   No repairable patterns found. Falling back to template generator …")
                use_fallback = True
