...
def model_execute():
    ...
    # 1. Pre-extract BW for conversions
    staged_params = staged.get("parameters", {})
    bw_param = staged_params.get("BW", {})
    BW = float(bw_param.get("value") if isinstance(bw_param, dict) else bw_param) if bw_param else default_vals["BW"]

    # 2. Pre-extract QCC for conversions
    qcc_param = staged_params.get("QCC", {})
    QCC = float(qcc_param.get("value") if isinstance(qcc_param, dict) else qcc_param) if qcc_param else default_vals["QCC"]
    qcc_unit = qcc_param.get("unit", "L/h/kg") if isinstance(qcc_param, dict) else "L/h/kg"
    if qcc_unit.lower() == "l/h":
        QCC = QCC / BW

    # 3. Process all parameters with unit conversions
    flat_params = {}
    for k in default_vals.keys():
        v = staged_params.get(k)
        val = None
        unit = ""
        ...
        if v is None:
            ...
            val = v.get("value")
            unit = v.get("unit", "")
            val = float(val) if val is not None else default_vals[k]
        else:
            try:
                val = float(v)
        ...
        # Standardize units for R solver
        if k == "QCC":
            val = QCC # Already converted to L/h/kg
        elif k == "Qliver":
            if unit.lower() == "l/h":
                val = (val / (QCC * BW)) * 100
            elif unit.lower() == "%qcc/bw":
                val = val * BW
        elif k == "CL_hep":
    ...
    # Dose conversion to absolute mg
    dose_val = float(staged.get("dose", 50.0))
    dose_unit = staged.get("dose_unit", "mg/kg").lower()
    
    if dose_unit == "mg/kg":
        dose_val = dose_val * BW
    elif dose_unit == "ug/kg":
        dose_val = (dose_val * BW) / 1000
    elif dose_unit == "ug":
    ...
    r_input = {
        ...
        "ode_method":        data.get("ode_method", "lsoda"),
    ...
    try:
        from r_code_gen import generate_standalone_r_script
        r_code = generate_standalone_r_script(staged, flat_params, dose_val, compound, paper_id, safe_slug)
    except Exception as e:
        r_code = f"# Error generating standalone script: {str(e)}\n"
...
@app.route("/api/model/generate_rcode_only", methods=["POST"])
def generate_rcode_only():
    """Generates the standalone R script and saves it without executing."""
    import json, re
    data   = request.get_json(force=True)
    staged = data.get("staged")
    if not staged:
    ...
    paper_id  = staged.get("paper_id", "unknown")
    compound  = staged.get("compound", "Compound")
    safe_slug = re.sub(r"[^a-zA-Z0-9_-]", "_", paper_id)[:50]
    ...
    file_path = output_dir / f"{safe_slug}_model.R"
    with open(file_path, "w") as f:
        f.write(r_code)
        
    return jsonify({"success": True, "path": str(file_path), "r_code": r_code})

@app.route("/api/model/save_rcode", methods=["POST"])
def save_rcode():
    """Save the generated R code to a local file in the project directory."""
    data = request.get_json(force=True)
